...
public class Example {
	public static void main(String[] args) {
		...
		cmp.setLayout(new MigLayout("insets 5, gap 0 0","[fill,grow,center][30][fill,grow,center][30]","[fill,grow][fill,grow][fill,grow][fill,grow][fill,grow][fill,grow]"));
		//datetime
		XDateTime date = new XDateTime(cmp); date.setLayoutData("cell 0 0, width 10:150:,height 23!, span 4"); date.setInput(System.currentTimeMillis());
		//spinner 1
		XSpinner<BigDecimal> spinner1 = new XSpinner<>(cmp, SWT.VERTICAL, new BigDecimalFormatter("##0.00","##0")); 
		spinner1.setLayoutData("cell 0 1, width 10:150:,height 23!, span 4"); 
		...
		spinner2.setLayoutData("cell 0 2, width 10:150:,height 50!, span 4"); 
		spinner2.setValue(new Long(999999999), new Long(-999999999), new Long(100), new Long(100));
		//slider
		XSlider slider = new XSlider(cmp, SWT.VERTICAL); slider.setLayoutData("cell 0 3, width 10:150:,height 24!, span 4");
		...
		Button btnOk1 = new Button(cmp, SWT.NONE); btnOk1.setLayoutData("cell 1 4, height 30!, bottom");
		...
		//infinite progress
		XInfiniteProgress progress1 = new XInfiniteProgress(cmp); progress1.setLayoutData("cell 2 4, width 100!,height 100!");
		Button btnOk2 = new Button(cmp, SWT.NONE); btnOk2.setLayoutData("cell 3 4, height 30!, bottom");
		btnOk2.setText("Start Progress"); btnOk2.addSelectionListener(new SelectionAdapter() {
			boolean running = false; @Override public void widgetSelected(SelectionEvent e) {
				super.widgetSelected(e); if (!running) {
					running = true;	progress1.start(); btnOk2.setText("Start Progress");
				} else {
					running = false; progress1.stop(); btnOk2.setText("Stop Progress");
				}
